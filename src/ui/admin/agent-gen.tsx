...
import type { SimAgent, WorldSnapshot } from '@/core/world'
import {
  UserPlus,
  Users,
  Info,
...
type AgentGeneratorPanelProps = {
  worldId: string
  world: WorldSnapshot
  onWorldUpdate?: (world: WorldSnapshot) => void
}
...
export function AgentGeneratorPanel({ worldId, world, onWorldUpdate }: AgentGeneratorPanelProps) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const existingAgents = world.agents.npcs.map((a: SimAgent) => ({
        seed: a.id,
        name: a.name,
        occupation: a.occupation,
      }))

      const response = await fetch('/api/sim/agents', {
        method: 'POST',
      ...
      const newAgent: SimAgent = data.agent
      console.log('Agent generated:', newAgent.name)
              ...
              <div key={index} className="rounded-lg border border-slate-200 bg-slate-50 shadow-sm transition-colors">
                ...
                {isExpanded && (
                  <div className="border-t border-slate-200 px-4 pb-4 pt-3">
                    {(agent.voice || agent.approach || agent.philosophy) && (
                      <div className="mb-3 space-y-1.5 rounded-md bg-slate-100 p-3 text-xs">
                        ...
                        {agent.philosophy && (
                          <div className="flex items-start gap-2">
                            ...
                            <span className="italic text-slate-500">{agent.philosophy}</span>
                          </div>
                        )}
                      </div>
                    )}
                    ...
                    <div className="space-y-3 text-sm">
                      <div>
                        ...
                        <div className="mt-1.5 grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-slate-500">
                          <div className="flex justify-between">
                            <span>Openness</span>
                            <span className="text-slate-600">{(agent.traits.openness * 100).toFixed(0)}%</span>
                          </div>
                          <div className="flex justify-between">
                            <span>Stability</span>
                            <span className="text-slate-600">{(agent.traits.stability * 100).toFixed(0)}%</span>
                          </div>
                          <div className="flex justify-between">
                            <span>Attachment</span>
                            <span className="text-slate-600">{(agent.traits.attachment * 100).toFixed(0)}%</span>
                          </div>
                          <div className="flex justify-between">
                            <span>Agency</span>
                            <span className="text-slate-600">{(agent.traits.agency * 100).toFixed(0)}%</span>
                          </div>
                          <div className="flex justify-between">
                            <span>Empathy</span>
                            <span className="text-slate-600">{(agent.traits.empathy * 100).toFixed(0)}%</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="mt-1.5 grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-slate-500">
                          <div className="flex justify-between">
                            <span>Energy</span>
                            <span className="text-emerald-600">{(agent.condition.energy * 100).toFixed(0)}%</span>
                          </div>
                          <div className="flex justify-between">
                            <span>Stress</span>
                            <span className="text-red-600">{(agent.condition.stress * 100).toFixed(0)}%</span>
                          </div>
                          <div className="flex justify-between">
                            <span>Sleep debt</span>
                            <span className="text-slate-600">{(agent.condition.sleep_debt * 100).toFixed(0)}%</span>
                          </div>
                          <div className="flex justify-between">
                            <span>Focus</span>
                            <span className="text-blue-600">{(agent.condition.focus * 100).toFixed(0)}%</span>
                          </div>
                          <div className="flex justify-between">
                            <span>Aging</span>
                            <span className="text-slate-600">{(agent.condition.aging_index * 100).toFixed(0)}%</span>
                          </div>
                        </div>
                      </div>
